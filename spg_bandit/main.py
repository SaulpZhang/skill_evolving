...
def main():
    ...
    for step in range(total_steps):
        ...
        if not is_warmup:
            ...
            log_metrics({"evolving/success_rate": success_count / bandit_done, "evolving/step": bandit_done})
    ...
    if args.eval:
        ...
        for step in range(n_bandit):
            ...
            if result["success"]:
                eval_success += 1
            log_metrics({"eval/success_rate": eval_success / (step + 1), "eval/step": step + 1})
            eval_records.append({
